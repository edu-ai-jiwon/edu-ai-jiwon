

<h1 align="center">Hi, I'm Jiwon Kim 👋</h1>

<h3 align="center">AI Service Developer | LLM · RAG · STT · Backend</h3>

<p align="center">
  한문교육 전공에서 AI 서비스 개발로 확장하며,<br/>
  사용자의 업무 흐름을 개선하는 AI 서비스를 만들고 있습니다.
</p>

<p align="center">
  <a href="포트폴리오링크">
    <img src="https://img.shields.io/badge/Portfolio-6A5ACD?style=for-the-badge&logo=Notion&logoColor=white"/>
  </a>
  <a href="https://github.com/YOUR_GITHUB_ID">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=GitHub&logoColor=white"/>
  </a>
  <a href="mailto:이메일">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=Gmail&logoColor=white"/>
  </a>
</p>

---

## About Me

- AI 서비스 개발자로 성장 중이며, LLM/RAG/STT 기반 서비스 구현에 관심이 있습니다.
- Django, FastAPI, React를 활용한 웹 서비스 개발 경험이 있습니다.
- 문서 기반 검색, 회의록 요약, 업무 자동화처럼 실제 사용자 흐름을 개선하는 프로젝트를 수행했습니다.
- 프로젝트 과정에서 데이터 구조 정리, API 연동 흐름 검토, README 및 발표자료 문서화를 담당했습니다.

---

## Featured Projects

<table>
  <tr>
    <td width="50%">
      <h3>회의피하지마 HPM</h3>
      <p><b>AI 회의 자동화 플랫폼</b></p>
      <p>
        STT와 LLM을 활용해 회의 준비자료 생성, 회의록 요약,
        업무 분배를 자동화하는 서비스입니다.
      </p>
      <p>
        <img src="https://img.shields.io/badge/Django-092E20?style=flat-square&logo=Django&logoColor=white"/>
        <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=FastAPI&logoColor=white"/>
        <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=React&logoColor=black"/>
        <img src="https://img.shields.io/badge/WhisperX-4B8BBE?style=flat-square"/>
        <img src="https://img.shields.io/badge/RAG-6A5ACD?style=flat-square"/>
        <img src="https://img.shields.io/badge/Qdrant-DC244C?style=flat-square"/>
      </p>
      <p>
        <a href="회의피하지마_레포링크">Repository</a>
      </p>
    </td>
    <td width="50%">
      <h3>Insurance Benefit Chatbot</h3>
      <p><b>RAG 기반 보험 가이드 챗봇</b></p>
      <p>
        한국 거주 외국인을 위한 보험 정보 안내 챗봇입니다.
        PDF 문서를 파싱하고 검색 기반 답변을 생성했습니다.
      </p>
      <p>
        <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=Python&logoColor=white"/>
        <img src="https://img.shields.io/badge/LangGraph-111111?style=flat-square"/>
        <img src="https://img.shields.io/badge/Chroma-5B5BD6?style=flat-square"/>
        <img src="https://img.shields.io/badge/OpenAI_API-412991?style=flat-square&logo=OpenAI&logoColor=white"/>
        <img src="https://img.shields.io/badge/BM25-555555?style=flat-square"/>
        <img src="https://img.shields.io/badge/RAG-6A5ACD?style=flat-square"/>
      </p>
      <p>
        <a href="보험챗봇_레포링크">Repository</a>
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>Burger Index & Crime Analysis</h3>
      <p><b>지역 치안 EDA 프로젝트</b></p>
      <p>
        버거 프랜차이즈 매장 수와 범죄 발생 데이터를 결합해
        지역별 치안 현황을 분석했습니다.
      </p>
      <p>
        <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=Python&logoColor=white"/>
        <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=Pandas&logoColor=white"/>
        <img src="https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=Selenium&logoColor=white"/>
        <img src="https://img.shields.io/badge/Data_Analysis-7952B3?style=flat-square"/>
      </p>
      <p>
        <a href="버거지수_레포링크">Repository</a>
      </p>
    </td>
    <td width="50%">
      <h3>Project Archive</h3>
      <p><b>Bootcamp Projects</b></p>
      <p>
        AI Camp 과정에서 수행한 웹 서비스, 데이터 분석,
        RAG 챗봇 프로젝트를 정리하고 있습니다.
      </p>
      <p>
        <img src="https://img.shields.io/badge/Documentation-000000?style=flat-square&logo=Notion&logoColor=white"/>
        <img src="https://img.shields.io/badge/Team_Project-FFB000?style=flat-square"/>
        <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=GitHub&logoColor=white"/>
      </p>
      <p>
        <a href="포트폴리오링크">Portfolio</a>
      </p>
    </td>
  </tr>
</table>

---

## My Role Highlights

### 회의피하지마 HPM
- 회의 목록 및 문서 조회 UI 개선
- LLM 응답 JSON 구조 정리
- 회의 준비자료 생성 흐름 검토
- 프론트엔드-백엔드 데이터 연동 구조 확인
- README 및 발표자료 문서화

### Insurance Benefit Chatbot
- PDF 문서 파싱 및 JSON 구조화
- BM25/Dense 검색 구조 실험
- 다국어 질의응답 평가
- 응답 품질 비교 및 문서화

### Burger Index & Crime Analysis
- KFC 매장 수 Selenium 크롤링
- JSON 기반 데이터 스키마 통일
- 분석용 데이터 정리 및 가공

---

## Tech Stack

### Backend & API
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=Python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Django-092E20?style=flat-square&logo=Django&logoColor=white"/>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=FastAPI&logoColor=white"/>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=MySQL&logoColor=white"/>
</p>

### AI / LLM
<p>
  <img src="https://img.shields.io/badge/LLM-111111?style=flat-square"/>
  <img src="https://img.shields.io/badge/RAG-6A5ACD?style=flat-square"/>
  <img src="https://img.shields.io/badge/vLLM-FF6F00?style=flat-square"/>
  <img src="https://img.shields.io/badge/WhisperX-4B8BBE?style=flat-square"/>
  <img src="https://img.shields.io/badge/Pyannote-7952B3?style=flat-square"/>
  <img src="https://img.shields.io/badge/OpenAI_API-412991?style=flat-square&logo=OpenAI&logoColor=white"/>
  <img src="https://img.shields.io/badge/Cohere_API-39594D?style=flat-square"/>
</p>

### Frontend
<p>
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=React&logoColor=black"/>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=TypeScript&logoColor=white"/>
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=TailwindCSS&logoColor=white"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=HTML5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=CSS3&logoColor=white"/>
</p>

### Data & Search
<p>
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=Pandas&logoColor=white"/>
  <img src="https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white"/>
  <img src="https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=Selenium&logoColor=white"/>
  <img src="https://img.shields.io/badge/Qdrant-DC244C?style=flat-square"/>
  <img src="https://img.shields.io/badge/Chroma-5B5BD6?style=flat-square"/>
</p>

### Collaboration
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=Git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=GitHub&logoColor=white"/>
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=Postman&logoColor=white"/>
  <img src="https://img.shields.io/badge/Notion-000000?style=flat-square&logo=Notion&logoColor=white"/>
</p>

---

## Currently Learning

- LLM/RAG 서비스 구조
- FastAPI & Django Backend
- Vector DB 기반 문서 검색
- React 기반 서비스 UI 구현

---
